...
from windowlab.metrics import metrics_row, positive_frequency_spectrum
...
def build_spectra() -> dict[str, list[tuple[float, float]]]:
    ...
    for name, builder in WINDOW_BUILDERS.items():
        ...
        db = [max(-120.0, 20.0 * __import__("math").log10(max(mag, 1e-12))) for mag in mags]
...
def main() -> int:
    ...
    with (ART / "window-metrics.csv").open("w", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=["name", "coherent_gain", "enbw_bins", "peak_sidelobe_db", "main_lobe_width"])
        writer.writeheader()
